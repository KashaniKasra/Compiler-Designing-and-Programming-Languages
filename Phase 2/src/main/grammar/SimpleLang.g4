...
selectionStatement returns [SelectionStatement selectionStatementRet]
    : i=If LeftParen e1=expression RightParen s1=statement { $selectionStatementRet = new SelectionStatement($e1.expressionRet, $s1.statementRet);
                                                             $selectionStatementRet.setIfLine($i.line); }
      (ei=ElseIf LeftParen e2=expression RightParen s2=statement { $selectionStatementRet.addElseIfCondition($e2.expressionRet);
                                                                   $selectionStatementRet.addElseIfBodyStatement($s2.statementRet);
                                                                   $selectionStatementRet.addElseIfLine($ei.line); })*
      (el=Else s3=statement { $selectionStatementRet.setElseStatement($s3.statementRet);
